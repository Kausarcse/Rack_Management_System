...
function show_user_access(user_type)
{
	if(user_type=='User')
	{  alert("yes");
			$("#for_user_access").show();
	}
	else{
		$("#for_user_access").hide();
	}
				...
				<form class="form-horizontal" action="" style="margin-top:10px;" name="edit_user_form" id="edit_user_form" enctype="multipart/form-data" data-parsley-validate>
					   ...
					   <div class="form-group form-group-sm" id="form-group_for_employee_name">
						...
						<label class="control-label col-sm-3" for="user_type" style="margin-right:0px; color:#00008B;">User Type:</label>
							<div class="col-sm-5">
								<select  class="form-control for_auto_complete" id="user_type" name="user_type" onchange="show_user_access(this.value)">
										...
										<option value="Restricted_User" <?php $options=$row['user_type']; if($options=="Restricted_User") echo 'selected="selected"'; ?> >Restricted User</option>
						...
						<div class="form-group form-group-sm" id="for_user_access" style="display: none;">
						<label class="control-label col-sm-3" for="user_access" style="margin-right:0px; color:#00008B;">User Access:</label>
							<div class="col-sm-5">
								<select  class="form-control" id="user_access" name="user_access">
											<option  value="<?php echo $row['user_access'] ?>" select="selected"><?php echo $row['user_access'] ?></option>
											<?php 
												 $sql = 'select distinct user_access from `user_login`';
												 $result= mysqli_query($con,$sql) or die(mysqli_error());
												 while( $row_for_user_access = mysqli_fetch_array( $result))
												 {

													 echo '<option value="'.$row_for_user_access['user_access'].'">'.$row_for_user_access['user_access'].'</option>';
						...
						</div> <!-- End of <div class="form-group form-group-sm" id="form-group_for_user_access"> -->
