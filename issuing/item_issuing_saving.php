...
else if(mysqli_num_rows($result) < 1)
{
	...
	{ 
				...
				else
				{
					...
					$datewise_transaction_sql = "UPDATE datewise_transaction_summary SET `total_issuing` = (`total_issuing` + $length) WHERE `transaction_date` = '$date_of_receipt' AND `work_order_number` = '$work_order_number' AND `gd_number` = '$gd_number' AND `customer_name` = '$customer_name' AND `construction` = '$construction' AND `composition` = '$composition' AND `weave` = '$weave' AND `shade` = '$shade' AND `finished_width` = '$finished_width' AND `finished_type` = '$finished_type' ";
				...
				{					
					...
					$activity_log_sql = "INSERT INTO activity_log (`barcode_code`, `construction`, `composition`, `weave`, `shade`, `finished_width`, `finished_type`, `transaction_type`, `activity_sql`, `response`, `recording_person_id`, `recording_person_name`, `recording_time`) VALUES ( '$barcode_code', '$construction', '$composition', '$weave', '$shade', '$finished_width', '$finished_type', 'issuing', '".mysqli_real_escape_string($con, $datewise_transaction_sql)."', 'failed', '$recording_person_id', '$recording_person_name', NOW() )";
	...
	else 
	{
			...
			$update_sql_statement="UPDATE item_info
									SET on_hand_stock = '$total_length',
