...
function reset_dropdown(select_element)
{
 ...
 function return_cubic_position()
 {
	 ...
	 $.ajax({
	 		url: 'report/find_cubic_position_by_rack_number_new.php',
	 		dataType: 'text',
	 		type: 'post',
	 		contentType: 'application/x-www-form-urlencoded',
 ...
 function return_color_wise_cubic_position()
 {
	 var rack_number = document.getElementById('rack_number').value;
	 var color = document.getElementById('color').value;
	 $.ajax({
	 		url: 'report/find_find_color_wise_cubic_position_by_rack_number_new.php',
	 		dataType: 'text',
	 		type: 'post',
	 		contentType: 'application/x-www-form-urlencoded',
	 		data: { rack_number: rack_number,color:color},
	 		success: function( data, textStatus, jQxhr )
	 		{
	 				//alert(data);
	 				document.getElementById('details_info').innerHTML=data;
		...
		<div class="panel panel-default"> <!-- This div will create a block/panel for FORM -->
				...
				<form class="form-horizontal row" action="" style="margin-top:10px;" method="post" name="attendance_management_device_information_form" id="attendance_management_device_information_form">
					...
					<div class="col-md-12">
						...
						<div class="form-group form-group-md" id="form-group_for_color">
							<label class="control-label col-md-3" for="customer_name" style="margin-right:0px; color:#00008B;">Color:</label>
								<div class="col-md-5 field_container">
									<select  class="form-control for_auto_complete" id="color" name="color" onchange="return_color_wise_cubic_position()">
												<option select="selected" value="select">Select Cubic Color</option>
												<option value="green">Green (Vacant)</option>
												<option value="yellow">Yellow (Partially)</option>
												<option value="red">Red (Fully)</option>
									</select>
								</div>
						</div>
							...
							<!-- <label class="control-label col-md-3" for="" style="margin-right:0px; color:#00008B;">Cubic Positions:</label> -->
								<div class="col-md-12 field_container">
									          <?php
													...
													if(($row2['kgs_ir'] ) > 0 && ($row2['kgs_ir'] ) < 1200)
													{
                                                    ...
                                                    //             // echo $row2['cubic_item_receiving'];
                                                    //             // echo $row3['cubic_item_issuing'];


													// 			if(($row2['length_ir'] - $row3['length_is']) > 0 && ($row2['length_ir'] - $row3['length_is']) < 2200)
													// 			{
													// 				echo '<button type="button" class="btn btn-warning btn-lg test"  value="'.$cubic_number.'" onClick="cubic_details(this.value)">'.$cubic_number.'</button>';
													// 			}
													// 			else if(($row2['length_ir'] - $row3['length_is']) >= 2200)
													// 			{
													// 				echo '<button type="button" class="btn btn-danger btn-lg test"  value="'.$cubic_number.'" onClick="cubic_details(this.value)">'.$cubic_number.'</button>';
													// 			}
													// 			else
   ...
   <div id="details_info_2"></div>
			...
			<div class="modal-dialog modal-md">
				...
				<div class="modal-header">
					<h1 class="modal-title"><strong>Stock Identity</strong></h1>
